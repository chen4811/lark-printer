  ...
  switch (fieldType) {
    ...
    case FieldType.MultiSelect.toString():
      return Array.isArray(valueData)
        ? valueData.map((item) => item.text || item.name).join("、")
        : valueData.text;
    case FieldType.CreatedTime.toString(): // 创建时间
    case FieldType.ModifiedTime.toString(): // 最后更新时间
    ...
    case FieldType.Phone.toString(): // 电话号码
      return valueData;
    case FieldType.Url.toString(): // 超链接
    ...
    case FieldType.Lookup.toString(): // 这里需要特殊处理
      if (valueData) {
